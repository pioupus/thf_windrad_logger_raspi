...
def write_down_log():
    ...
    with gzip.open(PROTOBUF_DATA_FOLDER+datetime.utcnow().strftime('%Y-%m-%dT%H_%M_%S_journalclt_out')+'.txt.gz', 'wb') as zipped_file:
        zipped_file.write(journal_ctl)
...
while 1:
    ...
    protobuf_dataset.service_uptime_s = int((datetime.now() - STARTTIME).total_seconds())
    protobuf_dataset.raspberry_uptime_s = int(uptime());
